...
@Component({
  selector: 'app-requests-user-ack',
  templateUrl: './requests-user-ack.component.html',
  styleUrls: ['./requests-user-ack.component.css'],
})

export class RequestsUserAckComponent implements OnInit {
      ...
      if (sId != null && Number(sId)) {
        this.data.getRequest(Number(sId)).subscribe(
          data => {
            ...
            this.isShowButton = (data['status'] == 6);
          }, error => {
            this.utils.promptError(error, 'Cannot get request');
  ...
  onClick() {
    let sId = this.route.snapshot.queryParamMap.get('id');
    if (sId != null && Number(sId)) {
      this.judDialog.showDialog('Information', 'The user acknowledge is submitting', 'wait', 'wait');
      this.data.userAck(Number(sId)).subscribe(
        data => {
          this.judDialog.showDialog('Information', 'The user acknowledge is submited successfully', 'succ', 'info');
          this.isShowButton = false;
          this.status_name = 'User Acknowledged';
        }, error => {
          // this.judDialog.showDialog('Error', 'Save Failed : ' + error.message, 'error', 'info');
          // this.judDialog.showDialog('Save Error', this.utils.parseError(error), 'error', 'info');
          this.judDialog.promptError(error, 'User acknowlegement failed');
        }
      )
    }
  }
