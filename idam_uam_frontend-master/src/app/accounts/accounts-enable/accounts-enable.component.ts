...
@Component({
  selector: 'app-accounts-enable',
  templateUrl: './accounts-enable.component.html',
  styleUrls: ['./accounts-enable.component.css'],
})

export class AccountsEnableComponent implements OnInit {

  @ViewChild('dt', { static: true }) userTable: Table;
  @ViewChild('judDialog', { static: true }) judDialog: JudDialogComponent;
  //first: number = 0;
  searchForm: FormGroup;
  enable_list: BaseRequest[] = [];
  ...
  onSelect(event) {
    //this.requests.splice(this.requests.indexOf(event), 1);
    this.enable_list.push(event);
  }

  onCancel(event) {
    //this.requests.push(event);
    this.enable_list.splice(this.enable_list.indexOf(event), 1);
  }

  showEnableButton(request) {
    //return (!(this.disable_list.indexOf(request) != -1 || request.account_status != 0))
    //let isInDisableList = this.disable_list.indexOf(request) == -1
    let isNotFoundInEnable = this.enable_list.find(x => x.id == request.id) == null;
    let isAccountDisable = (request.account_status == 1);
    return isNotFoundInEnable && isAccountDisable;
  }
  ...
  onConfirmEnable() {
    this.judDialog.showDialog('Confirmation', 'Are you sure to enable the users?', 'confirm', 'confirmSub');
  }

  onDialogSubmit() {
    let users = '';
    this.enable_list.forEach(element => {
    ...
    this.judDialog.showDialog('Information', 'The users are being enabled', 'wait', 'wait');
    this.data.enbleUserAccounts(userObject).subscribe(
      data => {
        this.judDialog.showDialog('Information', 'The users has been enabled successfully', 'succ', 'info');
        this.enable_list.forEach(element => {
          let id = element['id'];
          let enabledUser = this.tableItems.find(x => x.id == id);
          if (enabledUser) {
            let select_acc_status = this.searchForm.value['account_status'].value;
            if (select_acc_status == 'ALL') {
              enabledUser['account_status'] = 0;
              enabledUser['account_status_name'] = 'Active';
            } else {
              this.tableItems.splice(this.tableItems.indexOf(enabledUser), 1);
            }
          }
        });
        this.enable_list = [];
      }, error => {
        // this.judDialog.showDialog('Enable Error', this.utils.parseError(error), 'error', 'info');
        this.judDialog.promptError(error, 'Enable failed');
      }
    )
  }
