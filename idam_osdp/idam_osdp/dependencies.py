...
class BackendStorageImpl:
    ...
    def __init__(self, worker_ctx, osdp_config):
        self.worker_ctx = worker_ctx
        self.osdp_config = osdp_config
        self.base_dn = osdp_config['base_dn']
        self.hostname = osdp_config['hostname']
        self.portno = osdp_config['portno']
        self.username = osdp_config['username']
        self.pwd = osdp_config['pwd']
    ...
    def create_user(self, username, attrs):
        try:
            ...
            attrs['objectclass'] = ['dpPerson', 'organizationalPerson',
                                    'top', 'person', 'inetOrgPerson']
            user_dn = self._get_dn(username)
            # tmp_attrs = {key: value if attrs[key] is not None else '' for (
            #     key, value) in attrs.items()}
        ...
        try:
            ...
            for entry in entries:
                if 'attributes' in entry:
                    ...
                    att_names = ['cn', 'description', 'dpdeptid', 'dphkid', 'dprankcode', 'dpstaffgroup', 'employeeNumber', 'employeeType',
                                 'fullName', 'givenName', 'judnotesid', 'inetUserStatus', 'judinetmail', 'judromaid', 'juduid', 'JUDUserDPP', 'mail', 'sn', 'uid']
    ...
    def disable_user(self, username):
        return self.update_attribute(username, 'r', 'inetUserStatus', 'Inactive')

    def enable_user(self, username):
        return self.update_attribute(username, 'r', 'inetUserStatus', 'Active')
    ...
    def update_user(self, username, atts_new):
        ...
        try:
            attrs_old = self.search_user(username)
            if len(attrs_old) == 0:
                ...
                attrs_new_del = {key: value for (key, value) in attrs_new1.items() if not attrs_new1[key]}
                attrs_new1 = {key: value for (key, value) in attrs_new1.items() if attrs_new1[key]}

                if 'inetUserStatus' in attrs_old1:
                    attrs_old1.pop('inetUserStatus')

                attrs_dels = set(attrs_old1.keys()) - set(attrs_new1.keys()) | set([key for key in attrs_new_del.keys() if key in attrs_old1.keys()])
                ...
                if len(chgs) > 0:
                    ...
                    # return "2"
                    return {'success': True, 'remarks': "No changes detected"}
        except Exception as e:
            # return {'success': False, 'remarks': traceback.format_exc(e)}
            logger.error(str(e), exc_info=e)
            return {'success': False, 'remarks': str(e)}
...
class BackendStorage(DependencyProvider):
    ...
    def get_dependency(self, worker_ctx):
        return BackendStorageImpl(worker_ctx, self.container.config['OSDP_CONFIG'])
